...
import 'views/player_waiting_view.dart';
...
/// - /player_waiting?room={roomId}&id={playerId} -> PlayerWaitingView (플레이어 대기실)
/// - / -> LobbyView (방 생성/입장)
class AppRouter {
  static Route<dynamic> generateRoute(RouteSettings settings) {
    ...
    switch (uri.path) {
      case '/player_waiting':
        ...
        if (roomId == null || roomId.isEmpty || playerId == null || playerId.isEmpty) {
          return MaterialPageRoute(
            builder: (_) => const ErrorView(message: '파라미터가 누락되었습니다.'),
          );
        }
        return MaterialPageRoute(
          builder: (_) => PlayerWaitingView(roomId: roomId, playerId: playerId),
        );
      case '/host_waiting':
        final roomId = uri.queryParameters['room'];
